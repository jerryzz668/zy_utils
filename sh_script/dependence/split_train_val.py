...
from preprocessing.zy_utils import make_dir2, make_dir3, json_to_instance
...
def split_train_val(input_dir, ratio, coco_or_yolo):
    ...
    for sample in tqdm(train_sample):
        ...
        txt_path = sample[:sample.rindex('.')] + '.txt'
        if coco_or_yolo == 'coco':
            ...
            shutil.move(txt_path, os.path.join(output_path, 'labels', 'train'))
    ...
    for sample in tqdm(val_sample):
        ...
        if coco_or_yolo == 'coco':
            ...
            shutil.move(txt_path, os.path.join(output_path, 'labels', 'val'))
