...
def shape_to_mask(img_shape, points, shape_type=None,label = 1,
    ...
    if shape_type == 'circle':
        ...
        draw.ellipse([cx - dx, cy - dy, cx + dx, cy + dy], outline=1, fill=1)
        ...
        if label == '1':
            ...
            draw.line(xy=xy, fill=1, width=24)
        draw.line(xy=xy, fill=1, width=line_width)
...
def json2img(name_json,dir_img,dir_label):
    ...
    if data['imageData']:
        ...
        imagePath = os.path.join(os.path.dirname(json_file),"a.BMP")
    ...
    # else:
    #     src_RGB=img
    PIL.Image.fromarray(img).save(osp.join(dir_img, "test.bmp"))
...
def json_to_dataset_seg(Fs_Root_path,dir_json):
    ...
    out_image_path=train_seg_path+"\\image_baidian_gua"
    if not osp.exists(out_image_path):
        os.mkdir(out_image_path)
    out_label_path=train_seg_path+"\\label_baidian_gua"
...
#使用示例
Fs_Root_path='E:\\json_test'#根目录
Json_path_seg='E:\\json_test'#分割网Json文件夹路径
