...
# import yaml
import numpy as np
Remove_label=["zangwu","maoxu"]
def shape_to_mask(img_shape, points, shape_type=None,label = 1,
                  line_width=8, point_size=5):
    ...
    if shape_type == 'circle':
        ...
        if label == '1':
            draw.line(xy=xy, fill=1, width=6)
        elif label == '2':
            draw.line(xy=xy, fill=1, width=8)
        elif label == '3':
            draw.line(xy=xy, fill=1, width=10)
        elif label == '4':
            draw.line(xy=xy, fill=1, width=12)
        elif label == '5':
            draw.line(xy=xy, fill=1, width=14)
        elif label=='6':
            draw.line(xy=xy, fill=1, width=16)
        elif label == '7':
            draw.line(xy=xy, fill=1, width=18)
        elif label=='8':
            draw.line(xy=xy, fill=1, width=22)
        else:
            draw.line(xy=xy, fill=1, width=6)
        # draw.line(xy=xy, fill=1, width=line_width)
...
def shapes_to_label(img_shape, shapes, label_name_to_value, type='class'):
    ...
    for shape in shapes:
        ...
        mask = shape_to_mask(img_shape[:2], points, shape_type,label)
        cls[mask] = cls_id
        if type == 'instance':
            ins[mask] = ins_id
...
def json2img(name_json,dir_img,dir_label):
    ...
    # name_json,dir_img,dir_label = paras
    json_file = name_json
    ...
    img_dir=osp.join(dir_label, saved_name)
    img_gray=cv2.imread(r"%s"%img_dir,0)
    ret, img_bin = cv2.threshold(img_gray, 1, 255, cv2.THRESH_BINARY)
    cv2.imwrite(osp.join(dir_label, saved_name),img_bin)


    imgbin=cv2.imread(osp.join(dir_label, saved_name))
    print("img.shape():{}".format(len(img.shape)))
    if len(img.shape)==2:
        src_RGB = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    else:
        src_RGB=img
    PIL.Image.fromarray(src_RGB).save(osp.join(dir_img, saved_name))


def json_to_dataset_seg(Fs_Root_path,dir_json):
    train_seg_path=Fs_Root_path
    if not osp.exists(train_seg_path):
        os.mkdir(train_seg_path)
    out_image_path=train_seg_path+"\\image_huawei_AC_20201209"
    if not osp.exists(out_image_path):
        os.mkdir(out_image_path)
    out_label_path=train_seg_path+"\\label_huawei_AC_20201209"
    ...
    for i in range(0, len(count)):
        ...
        if os.path.isfile(path):
            if path.split('.')[-1]=="json":
                # paras.append((path,out_image_path,out_label_path))
                json2img(path,out_image_path,out_label_path)
    # print(paras)
    # pool = multiprocessing.Pool(processes=1) # 创建进程个数
    # pool.map(json2img,paras)
    # pool.close()
    # pool.join()

#使用示例
Fs_Root_path='G:\\huawei\\qingxi_shuju\\json_source\\wkx\\feiguaijiao\\'#根目录
Json_path_seg='G:\\huawei\\qingxi_shuju\\json_source\\wkx\\feiguaijiao\\AC_new_json'#分割网Json文件夹路径
