pt = r'D:\work\data\microsoft\jalama\ceshiji\dm\yt\yt_json'
import os
import json
...
def def_new_json(new_name,out_p,w,h,shapes):
    new_json = {}
    new_json['flags'] = {}
    ...
    save_json(new_json,out_p)
    # print('生成了',os.path.join(out_p,new_name))
    return new_json
import cv2
def fy(x,y,points,w,h):
    x0=0
    y0=0
    #print('jjj',y)
    if x=='1':
...
cut_json_p = r'D:\work\data\microsoft\jalama\test1231\testdataset\prejson'#切图的json文件，不可以带与图像混合存放
source_img_p = r'D:\work\data\microsoft\jalama\test1231\testdataset\dm1231'#原图的图像文件
save_p = r'D:\work\data\microsoft\jalama\test1231\testdataset\mergepre'#合并图的结果文件
img_dic = {}
...
for img_one in img_dic:
    ...
    for json_one in img_dic[img_one]:
        ...
        for k in shapes_json:
            ...
            print(x,y,'--')
            new_points = fy(x,y,points,w,h)
            k['points']=new_points
            shapes.append(k)
    json_name = img_one.replace('.jpg','.json')
    save_name = os.path.join(save_p,json_name)
    print(save_name)
    def_new_json(new_name=img_one,out_p = save_name,w=w,h=h,shapes=shapes)
