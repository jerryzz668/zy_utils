...
parser.add_argument('--test_dir', default='/home/jerry/Desktop/code/MPRNet/Deraining/results/Rain100H', type=str, help='image file which need to be test')
parser.add_argument('--gt_dir', default='/home/jerry/Desktop/code/MPRNet/Deraining/Datasets/test/Rain100H/target', type=str, help='image groundtruth file')
...
class SSIM():
    ...
    def run(self, imgx, imgy):
        ...
        X_2 = cv2.multiply(imgx, imgx, dtype=cv2.CV_32F)  # imgx sqr
        Y_2 = cv2.multiply(imgy, imgy, dtype=cv2.CV_32F)  # imgy sqr
        X_Y = cv2.multiply(imgx, imgy, dtype=cv2.CV_32F)  # imgx_imgy

        mu1 = cv2.GaussianBlur(imgx, (self.filterSize, self.filterSize), self.filterX)  # GaussianBlur()函数用高斯滤波器（GaussianFilter）对图像进行平滑处理。相当于求期望
        mu2 = cv2.GaussianBlur(imgy, (self.filterSize, self.filterSize), self.filterX)

        mu1_2 = cv2.multiply(mu1, mu1, dtype=cv2.CV_32F)  # mul1 sqr
        mu2_2 = cv2.multiply(mu2, mu2, dtype=cv2.CV_32F)  # mul2 sqr
        mu1_mu2 = cv2.multiply(mu1, mu2, dtype=cv2.CV_32F)  # mul1 mul2
