...
#
#     return img,labelimg
def save_json(inputlabelName,outputlabelName):
    json_data = json_to_instance(inputlabelName)
...
def ImageEnhance_seg(Fs_Root_path,sharpen=False,sp_noise=False,gasuss_noise=False,gamma=False,GaussianBlurring=False,equalizeHist=False,adaptivehistogram=False,Brightness=False):
    ...
    labelDirPath=Fs_Root_path+"/"
    imgFiles=[]
    for file_name in sorted(os.listdir(imgDirPath)):
        if file_name[file_name.rindex('.')+1:] in IMG_TYPES:
            imgFiles.append(file_name)
    # imgFiles = sorted(os.listdir(imgDirPath))
    # labelFiles = sorted(os.listdir(labelDirPath))
    RandomImgFiles=RandomSelectionFile(imgFiles,0.7)
    for imgName in RandomImgFiles:
        imgPath=os.path.join(imgDirPath, imgName)
        labelme=imgName[:-4]+'.json'
        ...
        if Brightness:
            ...
            flag2,Brightness_img2=Contrast_and_Brightness_enhance(img,0.8,0.8)
            Brightness_img_path = imgPath[:-4]+'_brightness.jpg'
            Brightness_label_path = labelPath.split(".")[0]+'_brightness.json'
            Brightness_img_path2 = imgPath[:-4]+'_brightness2.jpg'
            Brightness_label_path2 = labelPath.split(".")[0]+'_brightness2.json'
...
#使用示例
Fs_Root_path='/home/adt/data/data/huawei/huaweiCjian/20210913/侧面/select_hua_sha/train/crop_7682'
ImageEnhance_seg(Fs_Root_path,sharpen=False,sp_noise=False,gasuss_noise=False,gamma=False,
GaussianBlurring=False,equalizeHist=False,adaptivehistogram=False,Brightness=True)
